...
use super::heading::{Heading, Server, heading};
use super::*;
...
impl Render for AddServerForm {
    fn render(&mut self, _window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        let (server, failed) = self.standing(cx);
        let connecting = server == Server::Connecting;
        let reauthorize = server == (Server::Refused { oauth: true });
        let as_saved = reauthorize && self.unchanged(cx);
        let Heading {
            title,
            subtitle,
            action,
        } = heading(server, opened);
